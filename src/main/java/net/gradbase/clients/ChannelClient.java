...
public class ChannelClient extends BaseClient {
	...
	public JSONObject create(String subscriptionPassword, List<Map<String, String>> topics, Boolean hasPresharedKey,
		...
		for (int i = 0; i < topics.size(); i++) {
			...
			JSONObject topic = new JSONObject().put("type", t.get("type")).put("source", t.get("source"));
			topicsArray.put(topic);
		}

		JSONObject body = new JSONObject().put("topics", topicsArray);
	...
	/**
	 * Write data to a channel with address channel address. Write permission is
	 * mandatory. The type and metadata fields are not encrypted to have a
	 * possibility to search for events. The payload is stored encrypted for
	 * encrypted channels.
	 * 
	...
	public ChannelData write(String channelAddress, String type, String metadata, JSONObject payload)
		...
		JSONObject json = new JSONObject().put("type", type).put("created", nowAsISO).put("metadata", metadata)
				.put("payload", payload);

		System.out.println("Message send to channel: " + payload + " " + nowAsISO);
	...
	/**
	 * Get data from the channel with address channel address. The first possible
	 * message a subscriber can receive is the time the subscription got approved
	 * all messages before are not received. Read permission is mandatory.
	 * 
	 * @param channelAddress
	 * @param limit
	 * @param index
	...
	/**
	 * Get all data of a channel using a shared key (in case of encrypted channels).
	 * Mainly used from auditors to evaluate a log stream.
	 * 
	 * @param channelAddress
	 * @param presharedKey
	 * @throws InvalidAPIResponseException
	...
	/**
	 * Validates channel data by comparing the log of each link with the data on the
	 * tangle.
	 * 
	 * @param channelAddress
	 * @param datas
	...
	/**
	 * The user can decide to re-import the data from the Tangle into the database.
	 * A reason for it could be a malicious state of the data.
	 * 
	 * @param channelAddress
	 * @param body
	 * @throws InvalidAPIResponseException
	...
	/**
	 * Search for a channel. A client can search for a channel which it is
	 * interested in.
	 * 
	 * @param author
	 * @param authorId
	...
	public List<ChannelInfo> search(String author, String authorId, String topicType, String topicSource, Date created,
			...
			URISyntaxException, ParseException, InvalidAPIResponseException {
	...
	/**
	 * Add an existing channel into the database. Clients are able to add existing
	 * channels into the database so others can subscribe to them. This will be
	 * automatically called when a channel will be created.
	 * 
	 * @param info
	 * @throws InvalidAPIResponseException
	 */
	...
	/**
	 * Update channel information. The author of a channel can update topics of a
	 * channel.
	 * 
	 * @param info
	 */
	...
	/**
	 * Get all subscriptions of a channel. Use the is-authorized query parameter to
	 * filter for authorized subscriptions.
	 * 
	 * @param channelAddress
	 * @param isAuthorized
	 * @throws InvalidAPIResponseException
	...
	/**
	 * Request subscription to a channel with address channel-address. A client can
	 * request a subscription to a channel which it then is able to read/write from.
	 * 
	 * @param channelAddress
	 * @param options
	 * @throws InvalidAPIResponseException
	...
	/**
	 * Authorize a subscription to a channel with address channel-address. The
	 * author of a channel can authorize a subscriber to read/write from a channel.
	 * Eventually after verifying its identity (using the Ecommerce-SSI Bridge).
	 * 
	 * @param channelAddress
	 * @param subId
	 * @throws InvalidAPIResponseException
	...
	/**
	 * Revoke subscription to a channel. Only the author of a channel can revoke a
	 * subscription from a channel.
	 * 
	 * @param channelAddress
	 * @param subId
	 * @throws InvalidAPIResponseException
	...
	/**
	 * Adds an existing subscription (e.g. the subscription was not created with the
	 * api but locally.)
	 * 
	 * @param channelAddress
	 * @param id

...
public class AuthorizeToChannel {
	...
	public static void main(String args[]) throws Exception {
		// In this example we will use two instances of the ChannelClient() both will
		...
		// Creating a channel owner who creates the channel and a channel user who will
		// be authorized to read the channel
		// We will use two instances of the channel api client. One is getting
		// authorized by the owner and the other one by the user.
		JSONObject jsonClaim = new JSONObject().put("type", "Person").put("name", "Driver Bob");

		final String randomUsernameOwner = Utils.getRandomUsernameOfLength(5) + "owner";
		System.out.println("username: " + randomUsernameOwner);
		JSONObject channelOwner = identityClient.create(randomUsernameOwner, new Claim(jsonClaim));
		...
		JSONObject jsonClaim2 = new JSONObject().put("type", "Person").put("name", "Passenger Mike");

		final String randomUsernameUser = Utils.getRandomUsernameOfLength(5) + "user";
		System.out.println("username: " + randomUsernameUser);
		JSONObject channelUser = identityClient.create(randomUsernameUser, new Claim(jsonClaim2));
		...
		// The owner creates a channel where he/she want to publish data of type
		// 'example-data'.
		...
		// This attempt to read the channel will fail because the channel user is not
		// authorized to read the channel.
		try {
		...
		// Request subscription to the channel as the user. The returned
		// subscriptionLink can be used to authorize the user to the channel.
		...
		// Writing data to channel as the channel owner. Make sure to authorize
		// potential channel readers beforehand.
		System.out.println("Writing to channel 2nd time...");
		ownerClient.write(channelAddress, "log", null, new JSONObject().put("log", "This is log number 2"));

...
public class TrustedAuthorities {
	...
	public static void main(String args[]) throws Exception {
		...
		JSONObject driverIdentity = client.create(username, claim);
		...
		// List all trusted authorities, currently only one authority is trusted for
		// issuing credentials
		List<String> trustedAuthorities = client.getTrustedAuthorities();
		System.out.println(trustedAuthorities);

		VerifiableCredential driverCredential = client.createCredential(identityCredential,
				driverIdentity.getJSONObject("doc").getString("id"), CredentialType.BASIC_IDENTITY,
				new Claim(new JSONObject().put("type", "Person").put("driveAllowance", false).put("issuanceDate",
						"2014-09-11T11:00:00Z")));
		...
		// This drivers license will not be trusted because it was not added as an
		// trusted authority by us.
		...
		System.out.println(externalCredential.toJson());
		...
		try {
			...
			System.out.println("Trusted root status was not there before runtime");
		}
